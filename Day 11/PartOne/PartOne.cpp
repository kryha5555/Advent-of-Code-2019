#include <iostream>
#include <vector>
#define GRIDX 76 //hardcoded sizes, 100 is safe
#define GRIDY 58 //                 100 is safe
#define POSX 44  //                  50 is safe
#define POSY 4   //                  33 is safe
...
int main()
{
	...
	input.insert(input.end(), 470, 0); // 470 is minimal length of extra memory for this input
	...
	while (true)
	{
		...
		if (step == 2) // every 2 outputs
		{
			mask[posX][posY] = 1; // set panel as painted
			grid[posX][posY] = output[output.size() - 2]; // paint panel in according color

			output[output.size() - 1] ? dir = (dir + 1) % 4 : (!dir ? dir = 3 : dir = (dir - 1) % 4); // change direction by +1 or -1
	...
	for (auto i = 0; i < GRIDX; i++)
		for (auto j = 0; j < GRIDY; j++)
			panelsPainted += mask[i][j];

	std::cout << panelsPainted;
}
