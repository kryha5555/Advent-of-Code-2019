...
int main()
{
	...
	while (true)
	{
		...
		case 2:
			...
			break;
		case 3:
			input[mode_1 == 1 ? i + 1 : (mode_1 == 2 ? relativeBase + input[i + 1] : input[i + 1])] = 0;
			i += 2;
			break;
		case 4:
	...
	for (int i = 0; i < output.size(); i++)
		if (output[i] == 10)
		{
			row = 0; // reset row number
			col++;   // jump to next column
		}
		else
		{
			grid[col][row] = output[i];
			row++;
		}
