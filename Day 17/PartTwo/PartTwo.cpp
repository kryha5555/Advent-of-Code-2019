...
int main()
{
	...
	while (true)
	{
		...
		case 2:
			...
			break;
		case 3:
			input[mode_1 == 1 ? i + 1 : (mode_1 == 2 ? relativeBase + input[i + 1] : input[i + 1])] = inputTape[inputPointer++];
			i += 2;
			break;
		case 4:
	...
	for (int i = 1837; i < output.size() - 1; i++)
		std::cout << char(output[i]);

	std::cout << std::endl << output.back();
}