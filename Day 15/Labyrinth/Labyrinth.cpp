...
int main()
{
	...
	while (true)
	{
		...
		case 3:
			...
			input[mode_1 == 1 ? i + 1 : (mode_1 == 2 ? relativeBase + input[i + 1] : input[i + 1])] = direction;
			i += 2;
			break;
		case 4:
			outState = input[mode_1 == 1 ? i + 1 : (mode_1 == 2 ? relativeBase + input[i + 1] : input[i + 1])];
			i += 2;
			dx = 0;
			dy = 0;
			switch (direction)
...
void drawMaze(int(&maze)[SIZE][SIZE], int oY, int oX)
{
	...
	for (int y = 0; y < SIZE; y++)
	{
		for (int x = 0; x < SIZE; x++)
		{
			if (maze[y][x] == -1) // wall
				temp = 219;
			else if (maze[y][x] == 0) // unknown 
				temp = 176;
			else if (maze[y][x] == 1) // blank space
				temp = ' ';
			else if (maze[y][x] == 2) // robot
				temp = 'R';
